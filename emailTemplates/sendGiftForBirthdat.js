...
const sendGiftForBirthdat = ({ firstName, lastName, code: { promocode, purcendDiscount} }) => {
    ...
    <p>${firstName} ${lastName} your Birthday discount code ${promocode}</p>
    ...
    <a style="padding:12px 18px; background-color: rgb(184, 237, 237);border-radius: 20px;"
        href="https://www.shop-hop.store/shoping?rewardcode=${promocode}">Adding Code</a>
    <p>At problem visit <span style="color:blue;font-size: 18px;">https://www.shop-hop.store/shoping</span> and
        insert code:
        <span style="font-size: 22px;">${promocode}</span>
    </p>
    <p>послучай наближаващия ви рожден ден печелите ${purcendDiscount}% отстъпка кода може да се използва в рамките на 7 дни</p>
</div>`
}
